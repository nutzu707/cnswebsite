...
export async function GET() {
    try {
        const { blobs } = await list({
            prefix: 'consiliu-de-administratie/',
        });
        ...
        const people = await Promise.all(
            blobs.map(async (blob) => {
                try {
                    ...
                    console.error(`Error parsing consiliu file ${blob.pathname}:`, error);
                    return null;
                }
            })
        );
        ...
        console.error("Error reading consiliu-de-administratie from blob storage:", error);
