...
export async function GET() {
    try {
        const files = await listFromR2('consiliu-de-administratie/');
        ...
        const people = await Promise.all(
            files.map(async (file) => {
                try {
                    ...
                    console.error(`Error parsing consiliu file ${file.pathname}:`, error);
                    return null;
                }
            })
        );
        ...
        console.error("Error reading consiliu-de-administratie from R2 storage:", error);
