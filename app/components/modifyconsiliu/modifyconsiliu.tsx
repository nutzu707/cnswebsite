...
const ModifyConsiliu = () => {
    ...
    useEffect(() => {
        const fetchPeople = async () => {
            try {
                const response = await fetch('/api/consiliu-de-administratie');
                ...
                console.error('Error loading consiliu:', error);
                setPeople([]);
            } finally {
                setLoading(false);
            }
    ...
    const deletePerson = async (index: number) => {
        ...
        if (filename) {
            try {
                await deletePersonFile('consiliu-de-administratie', filename);
            } catch (e) {
    ...
    const saveAll = async () => {
        const tasks = people.map(async (p) => {
            ...
            await Uploadconsiliupersontoserver(fileName, blob);
        });
